 ...
 * composition is alive. Never scrolls on first composition or nav-return (the screen is recreated
 * and `remember` resets), so returning from a detail won't auto-scroll. For actions that don't
 * change sort/filter (e.g. pull-to-refresh) pass a token that only increments then.
 *
