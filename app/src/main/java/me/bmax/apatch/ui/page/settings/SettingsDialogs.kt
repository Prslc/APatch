...
fun SettingsDialogOverlay(uiState: SettingsUiState, viewModel: SettingsViewModel) {
    when (uiState.currentDialog) {
        SettingDialogType.RESET_SU_PATH -> ResetSUPathDialog(viewModel)
        SettingDialogType.CLEAR_CACHE -> ClearDialog(
...
@Composable
private fun LogDialog(viewModel: SettingsViewModel) {
...
@Composable
private fun ResetSUPathDialog(viewModel: SettingsViewModel) {
    val context = LocalContext.current
